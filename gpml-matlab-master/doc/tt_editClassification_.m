...
%Edit by TTruneh Jul 2021 
% See notebook below for clearer example !
...
%Gaussian Process classification is perhaps the most popular case of a GP model where the likelihood is not conjugate to the prior. 
%In the classification case, the likelihood is Bernoulli, and we must perform approximate inference over the values of the function. 
%There are many approaches to making such an approximation. 
% Use only the Laplace approximation to the likelihood
...
xtr = 10*sort(rand(ntr,6));                                     % sample dataset
